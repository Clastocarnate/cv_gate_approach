...
def apply_contour_detection(image, edges):
    ...
    for contour in contours:
        ...
        area = w * h
        if area > 70000:
            cv2.rectangle(contoured_image, (x, y), (x + w, y + h), (255, 0, 0), 2)  # Draw bounding box in blue
            cv2.putText(contoured_image, f'Area: {area}', (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)  # Print area on top of the bounding box
...
for image_path in image_files:
    ...
    # Display the result
    cv2.imshow("Contours and Bounding Boxes on Enhanced Image", contoured_image)
